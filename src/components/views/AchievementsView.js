...
export default function AchievementsView({
  ...
  return (
    <div className="view active" style={{ display: "flex", flexDirection: "column", gap: "24px" }}>
      ...
      {/* Performance Tiers Roadmap */}
      <div className="card" style={{ padding: "26px", borderRadius: "16px", border: "1px solid var(--border)", background: "var(--surface)" }}>
        <h3 className="section-title" style={{ marginTop: 0, marginBottom: "6px", fontSize: "18px" }}>
          🎯 Performance Bonus Tiers Roadmap
        </h3>
        <p style={{ color: "var(--text-muted)", fontSize: "13.5px", marginBottom: "24px" }}>
          Milestone targets and payout specifications across all 5 Performance Tiers.
        </p>

        <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
          {tierAchievements.map((tier, idx) => (
            <div
              key={idx}
              style={{
                padding: "20px",
                borderRadius: "14px",
                background: tier.isAchieved ? "rgba(16, 185, 129, 0.04)" : "var(--surface-2)",
                border: tier.isAchieved ? "1px solid rgba(16, 185, 129, 0.25)" : "1px solid var(--border)",
            ...
            >
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: "15px", marginBottom: "14px" }}>
                <div>
                  <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
                    <span style={{ fontSize: "20px" }}>{tier.isAchieved ? "🏅" : "🔒"}</span>
                    <span style={{ fontSize: "16px", fontWeight: "700", color: "#fff" }}>{tier.tierName}</span>
                    <span style={{
                      fontSize: "11px",
                      padding: "3px 10px",
                      borderRadius: "6px",
                      ...
                      background: tier.isAchieved ? "rgba(16, 185, 129, 0.15)" : "rgba(255, 255, 255, 0.08)",
                      color: tier.isAchieved ? "#10b981" : "var(--text-muted)",
                      border: tier.isAchieved ? "1px solid rgba(16, 185, 129, 0.3)" : "1px solid var(--border)"
                    }}>
                      {tier.statusText}
                    </span>
                  </div>
                  <div style={{ fontSize: "13px", color: "var(--text-muted)", marginTop: "4px" }}>
                    Required Team Business Target: <span className="mono" style={{ color: "#fff", fontWeight: "600" }}>{tier.targetVolume.toLocaleString()} USDT</span>
                  </div>
                </div>

                <div style={{ display: "flex", gap: "16px", flexWrap: "wrap", alignItems: "center" }}>
                  <div style={{ textAlign: "right" }}>
                    <div style={{ fontSize: "11px", color: "var(--text-muted)", textTransform: "uppercase" }}>Option A (Instant)</div>
                    <div className="mono" style={{ fontSize: "14px", fontWeight: "700", color: "var(--blue-bright)" }}>
                      +{tier.instantAmount} USDT
                    </div>
                  </div>
                  ...
                  <div style={{ textAlign: "right" }}>
                    <div style={{ fontSize: "11px", color: "var(--text-muted)", textTransform: "uppercase" }}>Option B (30-Day Stream)</div>
                    <div className="mono" style={{ fontSize: "14px", fontWeight: "700", color: "#f59e0b" }}>
                      {tier.dailyRate} USDT/day <span style={{ fontSize: "11px", color: "var(--text-muted)" }}>({tier.dailyRate * 30} Total)</span>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div>
                <div style={{ display: "flex", justifyContent: "space-between", fontSize: "12px", color: "var(--text-muted)", marginBottom: "6px" }}>
                  ...
                  <span className="mono">{tier.progressPct.toFixed(1)}% ({Math.min(totalTeamVolNum, tier.targetVolume).toLocaleString()} / {tier.targetVolume.toLocaleString()} USDT)</span>
                </div>
                <div style={{ height: "8px", background: "rgba(255, 255, 255, 0.08)", borderRadius: "4px", overflow: "hidden" }}>
                  <div style={{
                    height: "100%",
                    width: `${tier.progressPct}%`,
                    background: tier.isAchieved ? "linear-gradient(90deg, #10b981 0%, #34d399 100%)" : "linear-gradient(90deg, var(--blue-bright) 0%, #3b82f6 100%)",
              ...
              {tier.chosenOption && (
                <div style={{ marginTop: "12px", paddingTop: "12px", borderTop: "1px solid var(--border)", display: "flex", justifyContent: "space-between", alignItems: "center", fontSize: "12.5px" }}>
                  <span style={{ color: "var(--text-muted)" }}>Choice: <strong style={{ color: "#fff" }}>{tier.chosenOption}</strong></span>
                  <span style={{ color: "var(--text-muted)" }}>Total Payout Generated: <strong className="mono" style={{ color: "#10b981" }}>+{tier.totalPaidForTier.toFixed(2)} USDT</strong></span>
                </div>
              )}
            </div>
          ))}
